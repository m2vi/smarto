  ...
  return (
    <>
      <div className="w-full flex flex-col items-center">
        <div className="max-w-3xl mt-10 flex flex-row items-center w-full" style={{ height: "40px" }}>
          <BarButton className="rounded-r-0" onClick={() => loadVideo()}>
            <IoSearch className="h-4 w-4 text-primary-300" />
          </BarButton>
          <Bar placeholder="https://www.youtube.com/watch?v=dQw4w9WgXcQ" onKeyUp={handleKeyUp} ref={barRef} />
          <BarButton className="rounded-l-0" onClick={() => location.replace("https://github.com/m2vi/downloader/blob/main/README.md")}>
            <Tooltip>
              <IoInformationCircleOutline data-tip={t("pages.documentation")} className="h-4 w-4 text-primary-300" />
            </Tooltip>
          </BarButton>
        </div>
    ...
    <>
      <div className="w-full mt-9 px-9 max-w-3xl flex flex-col items-center">
        ...
        <div className="pt-7 flex justify-center items-center">
          ...
          <Button className="ml-1" color={"secondary"} onClick={() => dispatch({ type: "setOpen", value: true })}>
            {t("actions.configure")}
          </Button>
        </div>
      </div>
...
export const MainHolder = ({ info }: MainProps) => {
  ...
  const Bin = ({ error }: BinProps) => {
    return (
      <div className="w-full mt-9 px-9 max-w-3xl flex flex-col items-center">
        <div className="preview bg-primary-800 h-full w-full rounded-8 cursor-pointer" style={{ aspectRatio: "16 / 9" }}></div>
      </div>
    );
  };

  return <>{!valid || !info.success ? <Bin error={true} /> : <Main info={info} />}</>;
};
